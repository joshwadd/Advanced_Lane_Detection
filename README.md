...
To find the all the pixels associated with a lane lines a series of sliding windows are transversed along the height of the image so that it captures all of the pixels belonging to the lane within the windows. Each window is represented by the `Window` class found in `window.py`. The left and right lane windows are initialised in upon construction of the `LaneDectection` 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTczOTUzMzM1NV19
-->